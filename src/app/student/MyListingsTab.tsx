    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition"
        >
        ...
        <form action={handleCreate} className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-3">
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <input name="name" required placeholder="e.g. Handmade bookmark" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
            </div>
            <div className="w-28">
              <label className="block text-xs font-medium text-gray-500 mb-1">Price (WC)</label>
              <input name="price" type="number" min="1" required placeholder="500" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
            </div>
            <div className="w-24">
              <label className="block text-xs font-medium text-gray-500 mb-1">Quantity</label>
              <input name="quantity" type="number" min="1" required placeholder="5" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Description (optional)</label>
            <input name="description" placeholder="What are you selling?" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
          </div>
          {formError && <p className="text-xs text-red-500">{formError}</p>}
          <button type="submit" disabled={submitting} className="bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold py-2 rounded-xl transition disabled:opacity-50">
            ...
            <div key={l.id} className="bg-white rounded-2xl shadow-sm p-4 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm mt-1">
                  <span className="font-bold text-amber-600">{l.price.toLocaleString()} WC</span>
