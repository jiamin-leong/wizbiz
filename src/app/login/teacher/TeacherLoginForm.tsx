...
export default function TeacherLoginForm({ emailWorks }: { emailWorks: boolean }) {
  // One obvious path, and which one depends on what actually works. Until a
  // sending domain exists, an email link goes nowhere, so the password a
  // teacher set at sign-up leads instead.
  const [usePassword, setUsePassword] = useState(!emailWorks)
    ...
    <main className="grid-bg min-h-screen flex items-center justify-center px-6">
      <div className="card rounded-2xl p-8 w-full max-w-sm">
        ...
        <p className="text-center text-sm text-gray-400 mt-5">
          Got an invite code?{' '}
          <Link href="/signup" className="text-orange hover:underline font-medium">Set up your account</Link>
        </p>
        <Link href="/" className="block text-center text-sm text-gray-400 mt-2 hover:underline">
          Back
        </Link>
      </div>
    </main>
